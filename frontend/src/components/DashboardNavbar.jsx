...
const DashboardNavbar = ({ user, onLogout }) => {
    ...
    // Build a snapshot of the user's notification-relevant state
    const notifHash = useMemo(() => {
        if (!events || !currentUser) return '';
        // Create a hash from joined event IDs and their dates
        const joinedIds = [];
        events.forEach(event => {
            const isParticipant = event.participants?.some(p => (p.user?._id || p.user || p).toString() === currentUser._id?.toString());
            if (isParticipant) {
                joinedIds.push(event._id);
            }
        });
        return `${joinedIds.sort().join(',')}_${events.length}`;
    }, [events, currentUser]);
        ...
        <header className="navbar-header">
            <div className="navbar-container">
                    ...
                    <div className="user-profile-wrapper">
                        ...
                        >
                            ...
                            <div className="profile-text-info">
                                ...
                                <span className="user-displayRole">{user?.role || "Student"}</span>
                            </div>
                        </button>
                ...
                <nav className="navbar-navigation">
                    <NavLink to="/dashboard" end className={({ isActive }) => (isActive ? "nav-tab-link active" : "nav-tab-link")}>
                        <RiDashboardLine /> <span>Home</span>
                    </NavLink>
                    <NavLink to="/discover" className={({ isActive }) => (isActive ? "nav-tab-link active" : "nav-tab-link")}>
                        <RiCalendarEventLine /> <span>Browse</span>
                    </NavLink>
                    <NavLink to="/my-events" className={({ isActive }) => (isActive ? "nav-tab-link active" : "nav-tab-link")}>
                        <RiBookmarkLine /> <span>My Events</span>
                    </NavLink>
